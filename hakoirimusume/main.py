...
from hakoirimusume.message_handler import TalkMessageHandler

# get channel_secret and channel_access_token from your environment variable
channel_secret = config.get("line.bot.channel-secret")
channel_access_token = config.get("line.bot.channel-token")
...
message_handler = TalkMessageHandler(line_bot_api)
...
async def handle_callback(request: Request):
    ...
    for event in events:
        ...
        # )
        await message_handler.handle_message(event.message, event.source, event.reply_token)
